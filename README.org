...
1. *Generate a keypair* in the Secure Enclave and export its GPG public key.
2. *Sync* an encrypted password store over git (clone and pull).
3. *Browse* the password tree and *decrypt* entries on demand (biometric-gated use of the secure enclave private key).
...
identify a new GPG key with, and then click on the
...
Certainly you will want a backup of your password store. Of course,
you almost certainly are already making "backups" via git commits and
pushes to wherever you store your ~pass~ github repo. If you are not yet
using the git facility in ~pass~, stop right now and go and set that up
(instructions are [[https://www.passwordstore.org/][here]]).
 
With that taken care of, you need to import the public key you
...
Depending on your gpg setup, you will probably need to edit the trust
level of this key. You'll need the key's fingerprint (it's listed in
the app), and then edit it like this:

#+BEGIN_SRC sh
gpg --edit-key <new-fingerprint>
#+END_SRC
...
You'll need to combine all the fingerprints of keys you already use in ~pass~
with the new fingerprint you've added above with a command like:

#+BEGIN_SRC sh
pass init <existing-fingerprint1 existing-fingerprint2 … > <new-fingerprint>  
#+END_SRC
...
Of course you will need to repeat the gpg key import on any other
computer you use the same password store on.
...
~SE Pass~ app on your device. The ~Sync~ tab will let you specify a url, an
...
secure enclave ssh key by clicking the ~Generate SSH Key in Secure Enclave~
button. You'll need to get the public part of that ssh key
into your git repo server. If you're using github.com, instructions
for installing ssh keys are [[https://docs.github.com/en/authentication/connecting-to-github-with-ssh/adding-a-new-ssh-key-to-your-github-account][here]].

Whatever authentication you set up, hit the ~Clone~ button to actually
get the password store transferred to the ~SE Pass~ app.

Note that anytime you edit or add a password on your computer(s) using
~pass~ you will need to ~pass git push~ to your repo and then pull the
changes in ~SE Pass~ (there's a ~Refresh from Remote~ button).
...
~SE Pass~ should read any OpenPGP-encrypted password store.  It has been
tested with both the oldschool [[https://www.passwordstore.org/][pass]]
client (which generally uses ~gpg~ under the hood) and the
[[https://github.com/timvisee/prs][prs]] client (which uses the
[[https://github.com/rpgp/rpgp][rpgp]] / rpgpie library rather than ~gpg~). 
The two encode entries slightly differently on disk, and ~SE Pass~ handles
both formats:

- *Binary vs. ASCII-armored.* ~pass~ / ~gpg~ writes raw binary ~.gpg~ files by
  default, while ~prs~/rpgp writes ASCII-armored (=-----BEGIN PGP MESSAGE-----=)
  ones. ~SE Pass~ detects the format from the first byte and de-armors
  transparently, so either store decrypts without configuration.

- *Adding this device's key with ~prs~.* The re-encrypt flow above is the
  ~pass~ version. With ~prs~, import the exported public key and re-encrypt the
  existing secrets (~recipients add~ alone does *not* re-encrypt what's already
  there):

  #+begin_src sh
  prs recipients add --secret sepass-public.asc
...
- =App/SEPass/= — the SwiftUI app. Secure-Enclave key providers, key management,
  ...
  P-256 fallback (=#if targetEnvronment(simulator)=) lets the app run in the
  simulator.
...
Please feel free to review, comment, open issues, etc. We wrote and
released this because we could not quite believe there was not already
an app that uses the secure enclave and interacts with ~pass~, but WE
ARE NOT SECURITY EXPERTS. We've tried to be very striaghtforward in
our implementation, but are more than happy to accept criticism and
improvements from anyone with expertise in this area. Thanks!
