...
Note that ~pass~ / ~gpg~ writes raw binary ~.gpg~ files by default, while
~prs/rpgp~ writes ASCII-armored (=-----BEGIN PGP MESSAGE-----=) ones.  ~SE
Pass~ detects the format from the first byte and de-armors transparently, so
either store decrypts without configuration.

The re-encrypt flow described in the main section above is the ~pass~
version.  With ~prs~, import the exported public key and re-encrypt the
existing secrets (~recipients add~ alone does *not* re-encrypt what's
already there):
